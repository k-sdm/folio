...
// Hover-unhover cycles to advance the variant: 5 → JOURNEY_2, 20 → JOURNEY_3.
...
const levelFor = (count: number) => (count >= 20 ? 3 : count >= 5 ? 2 : 1);

/**
 * Journey — a self-contained project object. Starts on JOURNEY_1.webp and
 * advances to JOURNEY_2 after 5 hover-unhover cycles, then JOURNEY_3 after 20
 * total. On hover it crossfades to JOURNEY_hover.webp (the base fades out as the
...
export function Journey({ name, year }: { name: string; year: string }) {
  ...
  return (
    <div
      ...
      onMouseLeave={() => {
        setHovered(false);
        const next = Math.min(readCount() + 1, 20);
